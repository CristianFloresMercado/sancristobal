        ...
        <div class="page-header mb-4">
            <div class="page-title d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div>
                    ...
                    <p class="text-muted mb-0">Modificar datos de {{ $user->name }}</p>
                </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('role') is-invalid @enderror" name="role" required>
                                <option value="periodista" {{ old('role', $user->role) === 'periodista' ? 'selected' : '' }}>Periodista</option>
                                <option value="rrhh" {{ old('role', $user->role) === 'rrhh' ? 'selected' : '' }}>RRHH</option>
                                <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>Administrador</option>
                            </select>
                            @error('role') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Nueva contraseña (dejar vacío para mantener)</label>
